...
import { SearchEpisodesDto } from './dto/search-episodes.dto';
...
export class EpisodesController {
  ...
  @Get('search')
  @ApiOperation({
    summary: 'Search and filter episodes with cursor-based pagination',
    description: 'Search by title/description, filter by podcast and published status',
  })
  @ApiQuery({ name: 'search', required: false, description: 'Search term for title and description' })
  @ApiQuery({ name: 'podcastId', required: false, description: 'Filter by podcast ID' })
  ...
  @ApiResponse({ status: 400, description: 'Validation error' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  search(@Query() query: SearchEpisodesDto, @CurrentUser() user: JwtPayload): Promise<PaginatedResponseDto<EpisodeResponseDto>> {
